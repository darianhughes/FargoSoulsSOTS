...
namespace FargoSoulsSOTS.Content.Projectiles.Masomode
{
    public class PlasmaHookProj : ModProjectile
    {
        ...
        private void SpawnChargeDust(Color c)
        {
            ...
            {
                PixelDust
                    .Spawn(Projectile.Center - Projectile.velocity, 0, 0, Utils.NextVector2Circular(Main.rand, 3f, 3f), c, 12)
                    .scale = Utils.NextFloat(Main.rand, 1.25f, 1.5f);
            }
        }
        ...
        private void TryDeathBurst(Color c)
        {
            if (Projectile.ai[0] == 2 && Projectile.localAI[0] != -1f)
            {
                ...
                {
                    PixelDust
                        .Spawn(Projectile.Center - Projectile.velocity, 0, 0, Utils.NextVector2Circular(Main.rand, 8f, 8f), c, 12)
                        .scale = Utils.NextFloat(Main.rand, 1.5f, 2f);
                }
