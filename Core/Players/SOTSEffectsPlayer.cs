...
namespace SecretsOfTheSouls.Core.Players
{
    ...
    public class SOTSEffectsPlayer : ModPlayer
    {
        ...
        {
            if (FargoGlobalNPC.SpecificBossIsAlive(ref SOTSGlobalNPCEffects.excavatorBoss, ModContent.NPCType<Excavator>())
                && Player.Distance(Main.npc[SOTSGlobalNPCEffects.excavatorBoss].Center) < 3000)
            {
                if (WorldGen.crimson)
    ...
    public class PlayerCurseDebuffCounterLayer : PlayerDrawLayer
    {
        ...
        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            ...
            var modPlr = player.GetModPlayer<SOTSEffectsPlayer>();

            int height = 18;
            SOTSEffectsPlayer.DrawPermanentDebuffsForPlayer(
                player,
                Main.spriteBatch,
                Main.screenPosition,
                Color.White,
